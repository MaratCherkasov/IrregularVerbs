...
final class TrainViewController: UIViewController {
    ...
    private lazy var scoreLabel: UILabel = {
        ...
        let scoreLabel = NSLocalizedString("score: %d", comment: "")
        let formattedScoreLabel = String(format: scoreLabel, score)
        label.text = formattedScoreLabel
    ...
    private var count = 0 {
        didSet {
            ...
            checkButton.backgroundColor = .opaqueSeparator
    ...
    private var score = 0 {
        didSet {
            let label = NSLocalizedString("score: %d", comment: "")
            let formattedScoreLabel = String(format: label, score)
            scoreLabel.text = formattedScoreLabel
            print("score: \(score)")
        }
    }
    ...
    private func checkAction() {
        if checkAnswers() {
            if checkButton.backgroundColor != .red {
                score += 1
            }
            if currentVerb?.infinitive == dataSource.last?.infinitive {
                let message = NSLocalizedString("Number of points: %d", comment: "")
                let formattedMessage = String(format: message, score)
                let alert = UIAlertController(title: "Congratulations!".localized,
    ...
    private func checkAnswers() -> Bool {
        pastSimpleTextField.text?.lowercased().trimmingCharacters(in: .whitespacesAndNewlines) == currentVerb?.pastSimple.lowercased() &&
    ...
    private func setupConstraints() {
        ...
        checkButton.snp.makeConstraints { make in
            make.top.equalTo(participleTextField.snp.bottom).offset(90)
            make.trailing.leading.equalToSuperview().inset(edgeInsets)
        }
